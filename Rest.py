...
title_lbl = Label(win, text='Restaurant Billing System',
                  font=('', 55, 'bold', 'underline'), bg='skyblue')
...
def home_screen():
    ...
    login_btn = Button(frm, command=lambda: auth(), width=8,
                       text="login", font=('', 20, 'bold'), bd=10)
    reset_btn = Button(frm, command=lambda: reset_home(),
                       width=8, text="reset", font=('', 20, 'bold'), bd=10)
    ...
    def auth():
        ...
        if (len(u) == 0 or len(p) == 0):
            messagebox.showwarning(
                'Validation', "Username/Password cann't be empty")
        else:
...
def welcome_screen():
    ...
    logout_btn = Button(frm, command=lambda: logout(), width=8,
                        text="logout", font=('', 20, 'bold'), bd=10)
    logout_btn.place(relx=.88, y=1)

    billing_btn = Button(frm, command=lambda: billing(
    ), width=12, text="Billing", font=('', 20, 'bold'), bd=10)
    billing_btn.place(relx=.4, rely=.1)

    addItem_btn = Button(frm, command=lambda: addItem(
    ), width=12, text="Add Item", font=('', 20, 'bold'), bd=10)
    addItem_btn.place(relx=.4, rely=.25)

    editPrice_btn = Button(frm, command=lambda: editPrice(
    ), width=12, text="Edit Price", font=('', 20, 'bold'), bd=10)
...
def additem_screen():
    ...
    title_lbl = Label(frm, text="Add Item", font=(
        '', 50, 'bold', 'italic'), bg='pink', fg='green')
    ...
    back_btn = Button(frm, command=lambda: back(), width=6,
                      text="back", font=('', 20, 'bold'), bd=10)
    ...
    price_label = Label(frm, text="Price/Unit",
                        font=('', 20, 'bold'), bg='pink')
    price_entry = Entry(frm, font=('', 20, 'bold'), bd=10)

    login_btn = Button(frm, command=lambda: addItem_db(),
                       width=8, text="Add", font=('', 20, 'bold'), bd=10)
    reset_btn = Button(frm, command=lambda: reset_home(),
                       width=8, text="reset", font=('', 20, 'bold'), bd=10)
...
def editprice_screen():
    ...
    title_lbl = Label(frm, text="Edit Price", font=(
        '', 50, 'bold', 'italic'), bg='pink', fg='green')
    title_lbl.place(relx=.35, y=14)
    ...
    user_label = Label(frm, text="Select Item",
                       font=('', 20, 'bold'), bg='pink')
    ...
    def select_item(event):
        ...
        cur = con.cursor()
        cur.execute(
            "select item_price_unit from items where item_name=?", (item,))
        row = cur.fetchone()
        iprice = row[0]
        pass_entry.delete(0, END)
    ...
    login_btn = Button(frm, command=lambda: updatePrice_db(),
                       width=8, text="Update", font=('', 20, 'bold'), bd=10)
    reset_btn = Button(frm, command=lambda: reset_home(),
                       width=8, text="reset", font=('', 20, 'bold'), bd=10)
    ...
    def updatePrice_db():
        ...
        cur = con.cursor()
        cur.execute(
            "update items set item_price_unit=? where item_name=?", (price, item))
        con.commit()
        con.close()
...
def billing_screen():
    ...
    title_lbl = Label(frm, text="Billing", font=(
        '', 50, 'bold', 'italic'), bg='pink', fg='green')
    title_lbl.place(relx=.35, y=14)
    ...
    item_label = Label(frm, text="Select Item",
                       font=('', 15, 'bold'), bg='pink')
    ...
    add_btn = Button(frm, command=lambda: add_item_to_details(),
                     width=8, text="add", font=('', 15, 'bold'), bd=5)
    delete_btn = Button(frm, command=lambda: delete_item_to_details(
    ), width=8, text="delete", font=('', 15, 'bold'), bd=5)

    items_frm = Frame(frm, bg='white')
    items_frm.place(relx=.45, rely=.15, width=700, height=400)

    item_details = Label(items_frm, text="Billing Items",
                         font=('', 15, 'bold'), bg='white', fg='red')
    item_details.pack()

    bill_btn = Button(items_frm, command=lambda: final_bill(),
                      width=5, text="Bill", font=('', 15, 'bold'), bd=5)
    bill_btn.pack(side='bottom', anchor='e', padx=20, pady=20)

    Label(items_frm, text="Item", font=('', 13, 'bold'),
          bg='white', fg='black').place(relx=.05, rely=.07)
    Label(items_frm, text="Qty", font=('', 13, 'bold'),
          bg='white', fg='black').place(relx=.35, rely=.07)
    Label(items_frm, text="Price/Unit", font=('', 13, 'bold'),
          bg='white', fg='black').place(relx=.65, rely=.07)
    Label(items_frm, text="Amount", font=('', 13, 'bold'),
          bg='white', fg='black').place(relx=.9, rely=.07)

    date = str(datetime.now())
    Label(items_frm, text=f"{date}", font=('', 13, 'bold'),
          bg='white', fg='blue').place(relx=.01, rely=0)
    ...
    def show_billed_items():
        ...
        for item in billed_items:
            Label(items_frm, text=item, bg='white', fg='blue',
                  font=('', 12)).place(relx=.05, rely=.15+y)
            qty_price_amt = billed_items.get(item)
            x = 0
            for val in qty_price_amt:
                Label(items_frm, text=str(val), bg='white', fg='blue',
                      font=('', 12)).place(relx=.35+x, rely=.15+y)
                x = x+.3
            y = y+.1
